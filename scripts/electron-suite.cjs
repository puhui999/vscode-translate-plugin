...
exports.run = async function run() {
  ...
  try {
    ...
    await waitForReader(api, document, 3);
    await vscode.commands.executeCommand('commentTranslator.toggle');
    console.log('ELECTRON_SMOKE_PASSED: borderless reader DOM, unchanged source, custom provider settings, batched request, SQLite reuse, changed-comment-only request');
  } finally {
