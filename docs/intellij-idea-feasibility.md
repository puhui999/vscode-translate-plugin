# IntelliJ IDEA 版可行性与实现方案（历史）

> 本文保留实现前的调研和设计，不代表当前已实现范围。当前安装、使用与兼容性以 [IDEA README](../idea-plugin/README.md) 为准。`0.1.0` 曾采用上下 Block Inlay；根据用户实际体验反馈，`0.1.1` 改为默认原位译文，并将上下对照保留为可选模式。下文的“首版”“尚未实现”和原位显示仅作后续探索等表述，均属于当时的方案；Markdown 全文翻译尚未包含在当前 IDEA 版中。
